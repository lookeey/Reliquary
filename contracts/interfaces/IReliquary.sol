 ...
 * `rewardDebt` Amount of reward token accumalated before the position's entry or last harvest for each epoch.
 ...
 * `lastClaim` Timestamp of the last time rewards were claimed.
 */
struct PositionInfo {
    uint amount;
    uint[] rewardDebt;
 ...
 * `accRewardPerShare` Accumulated reward tokens per share of pool (1 / 1e12) for each epoch.
 ...
 * `levelPeriod` The maturity (in seconds) required to reach each Level.
...
/**
 * @notice Info for balances of a pool, according to epochs and in total.
 * `balances` Balances deposited at each epoch (`balance[0]` = balances deposited at first epoch).
 * `totalBalance` Total balance deposited in the pool (multipliers not included).
 */
struct TranchesInfo {
    uint[] balances;
    uint totalBalance;
